...
// Pillow-shaped non-activating overlay for transient feedback. Mirrors the
// pattern used in ShotMemo / PasteMemo so all three apps feel the same.
...
final class ToastCenter {
    ...
    /// 进程退出前同步清掉浮动面板。该面板的 collectionBehavior 跨所有
    /// Space，如果由 SIGKILL 直接释放，WindowServer 会做一帧全屏重绘
    /// （表现为屏幕闪一下）。在应用还活着的时候 orderOut 并释放，能彻底
    /// 跳过那一帧。
