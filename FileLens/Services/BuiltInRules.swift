...
enum BuiltInRules {
    /// 用户在 设置 → 工作区默认 中可调整这两个阈值,新建工作区使用当前值。
    /// 已建工作区不受影响 —— 用户可以在规则编辑器里单独改。
    static var defaultNewArrivalsDays: Int {
        UserDefaults.standard.object(forKey: "filelens.newArrivalsDays") as? Int ?? 7
    }
    static var defaultStaleDays: Int {
        UserDefaults.standard.object(forKey: "filelens.staleDays") as? Int ?? 30
    }
    ...
    static func all() -> [Rule] {
        let newDays = String(defaultNewArrivalsDays)
        let staleDays = String(defaultStaleDays)
        return [
            rule("Folders",      "#A78BFA", priority:  5, [c("kind", "is", "folder")]),
            rule("Installers",   "#3B82F6", priority: 10, [c("extension", "isAnyOf", "dmg,pkg,app")]),
            rule("Images",       "#10B981", priority: 20, [c("kind", "is", "image")]),
            rule("Videos",       "#EF4444", priority: 30, [c("kind", "is", "movie")]),
            rule("Audio",        "#F59E0B", priority: 40, [c("kind", "is", "audio")]),
            ...
            rule("Screenshots",  "#EC4899", priority: 90,
                 [c("name", "matches", "^(截屏|Screenshot|CleanShot|截图)")]),
            rule("Large files",  "#F97316", priority: 100, [c("size", ">", "500MB")]),
            rule("New arrivals", "#0EA5E9", priority: 110, [c("dateAdded", "inLastDays", newDays)]),
            rule("Stale",        "#9CA3AF", priority: 120, [c("dateAdded", "notInLastDays", staleDays)]),
